...
export class PaginationButtons extends Component {
    ...
    getPageNumbers = () => {
        if (this.props.pageCount < 4) {
            const fixedPages = [...Array(this.props.pageCount + 1).keys()].slice(1);
            return fixedPages;
        } else if (this.props.currentPage <= 4) {
            const firstPages = [1, 2, 3, 4, 5]
            return firstPages;
        } else if (this.props.currentPage > this.props.pageCount - 4) {
            const lastPages = [...Array(5).keys()].reverse().map(v => this.props.pageCount - v)
            return lastPages;
        } else {
            const middlePages = [this.props.currentPage - 1, this.props.currentPage, this.props.currentPage + 1]
            return middlePages;
        }
    }
    render() {
